...
./gradlew :app:koverHtmlReportDebug             # Generate debug unit-test coverage HTML report
./gradlew :app:koverLogDebug                    # Print debug unit-test coverage summary
...
Coverage reports are generated by Kover. After `JAVA_HOME` is set as shown above, run `./gradlew :app:koverHtmlReportDebug` and open `app/build/reports/kover/htmlDebug/index.html`.
